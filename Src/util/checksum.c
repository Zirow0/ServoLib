 ...
 * повинна бути парною. Якщо в даних парна кількість одиниць, біт парності = 0.
 * Якщо непарна, біт парності = 1.
 *
 * Приклад 1: data = 0b1010101010101010 (8 одиниць - парна кількість)
 *   Результат: 0 (парність правильна)
 *
 * Приклад 2: data = 0b1010101010101011 (9 одиниць - непарна кількість)
 *   Результат: 1 (потрібен біт парності для парності)
 *
 * Для AEAT-9922 SPI4-A:
 *   - Біт парності P знаходиться в біті 15 (старший біт першого байта)
 *   - Обчислення проводиться для всіх 16 біт (включно з P)
 *   - Якщо парність правильна, результат повинен бути 0
 */
uint8_t Checksum_EvenParity16(uint16_t data)
{
    uint8_t parity = 0;

    // Підрахувати кількість одиниць (population count / Hamming weight)
    // Використовуємо Brian Kernighan's алгоритм для ефективності
    while (data) {
        parity ^= 1;          // Інвертувати біт парності
        data &= (data - 1);   // Скинути найменший встановлений біт
    }
