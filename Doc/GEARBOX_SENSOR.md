...
> **НЕ РЕАЛІЗОВАНО.** Цей модуль описує заплановану функціональність, якої немає в кодовій базі.
> Жодного файлу `gearbox.h/c`, типу `Gearbox_Sensor_Handle_t` або будь-якої з описаних функцій не існує.
> Для роботи з редуктором використовуйте `Position_Sensor_Interface_t` напряму і рахуйте коефіцієнт передачі на рівні застосунку.
