...
### drv/sensor/ (Драйвери датчиків)

| Файл | Опис |
|------|------|
| `pos.h` / `pos.c` | Універсальна обгортка над датчиком положення з фільтрацією |
| `i2c.h` / `i2c.c` | Допоміжні функції для I2C датчиків (таймаути, повтори, статистика) |
| `as5600.h` / `as5600.c` | Драйвер 12-біт магнітного енкодера AS5600 (використовує `hwd_i2c.h`) |
