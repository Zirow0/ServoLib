 ...
 * Вимірювання: [θ] — абсолютний кут з лічильника енкодера
 *
 ...
 *   α(k+1) = α(k)
 *
 * Для розширення до 4-го стану змінити ENCODER_UKF_N_STATES,
 * state_func і meas_func у encoder_ukf.c, додати поля до
...
#define ENCODER_UKF_N_MEAS    1U   /**< θ */
...
#define ENCODER_UKF_R_THETA_DEFAULT   1e-4f   /**< шум вимірювання: позиція (рад²) */
...
typedef struct {
    ...
    float r_theta;   /**< шум вимірювання: позиція */
 ...
 *
 * buffer — зовнішній буфер пам'яті для ukf_t (нова архітектура ukf_mcu).
 * Розмір обчислюється макросом UKF_BUFFER_FLOATS.
 */
typedef struct {
    ukf_t ukf;
