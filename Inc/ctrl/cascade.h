 ...
 *   ff = ff_j * current_sp + ff_b * omega
 *   де ff_j [%/A], ff_b [%·с/рад] — коефіцієнти, що тюнуються.
 *   Результат додається до виходу trq-PID.
 *   У режимі TRQ ff_b не застосовується.
 *
 * Slew rate limiter:
...
typedef struct {
    ...
    float ff_j;       /**< FF інерції [%/А]: % команди на 1 А setpoint струму */
    float ff_b;       /**< FF тертя [%·с/рад]: % команди на 1 рад/с швидкості */
